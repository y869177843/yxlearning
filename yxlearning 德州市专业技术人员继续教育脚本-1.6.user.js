...
(function() {
    ...
    try {
        ...
        const INTERVAL_SWITCH_MODE = 5000;
        ...
        function watchFirstPlay() {
            ...
            if (videoElement) {
                ...
                }).observe(videoElement, { attributes: true, attributeFilter: ['paused'] });
                return;
            }
            // 兼容旧的播放器监听
            const wrap = document.querySelector('.bplayer-wrap');
        ...
        function switchPlaybackMode() {
            // 旧页面模式切换逻辑
            const isVideoModeActiveOld = document.querySelector("span[data-mode='video'].pv-active");
            if (isVideoModeActiveOld) {
                let switched = false;
                const btnOldSelector = document.querySelector("div.pv-setting-choose.pv-stream-select span:nth-child(2)");
                if (btnOldSelector && btnOldSelector.offsetParent !== null) {
                    console.log('[继续教育] 尝试旧网站方式切换到音频模式 (nth-child)。');
                    btnOldSelector.click();
                    switched = true;
                }
                setTimeout(() => {
                    const stillVideoMode = document.querySelector("span[data-mode='video'].pv-active");
                    const audioBtnNewSelector = document.querySelector("span[data-mode='audio']");
                    if (stillVideoMode && audioBtnNewSelector && audioBtnNewSelector.offsetParent !== null && !switched) {
                        console.log('[继续教育] 尝试旧网站方式切换到音频模式 (data-mode="audio")。');
                        audioBtnNewSelector.click();
                    }
                }, 500);
            }
 
            // **新页面模式切换逻辑：需要你提供元素信息才能添加**
            // 暂时不加，等待你的反馈
        }
        ...
        function clickSkipButton() {
            ...
            Array.from(document.getElementsByTagName('button'))
                 ...
                 .forEach(b=>{ console.log('[继续教育] 点击旧网站跳过按钮。'); b.click(); });
        ...
        setInterval(switchPlaybackMode, INTERVAL_SWITCH_MODE); // 模式切换需要你提供新网站元素信息
